...
class MainNode(Node):
    ...
    def platform_request(self, dst: PlatformCmd | None = None, offset: bool = False) -> int:
        '''
        call with dst to move
        call with offset to request platform offset at unload and move to unload
        '''
        req = Command.Request()
        ## Request (offset) cmd
        if dst is None and offset:
            req_dict = {
                "offset": True
            }
        ## Request (src,dst) cmd
    ...
    def update_text_info(self, level: PlatformCmd, put: bool) -> str:
        '''
            call put=False after arm detect pose to store current_level_text and update info\n
            call put=True after arm put_down pose to pop current_level_text and update info\n
            return first char of current_text
        '''
        print(f"current text: {self.current_text}")
        print(f"before current level: {self.current_level_text}")
        return_char = ""
        ## grab mode
        if not put:
            ...
            ## set return char when detect
            if self.current_level_text != "":
                return_char = self.current_level_text[0]
        ## put mode
            ...
            ## set text_info to pop char
            self.text_info[c] = level
            return_char = c
        print(f"after current level: {self.current_level_text}")
        print(f"text info: {self.text_info}")
        print(f"first char: {return_char}")
        return return_char
    ...
    def is_task_finish(self) -> bool:
        '''
            all text in unload
        '''
        return self.text_info['F'] == PlatformCmd.UNLOAD \
            and self.text_info['I'] == PlatformCmd.UNLOAD \
            and self.text_info['R'] == PlatformCmd.UNLOAD \
            and self.text_info['A'] == PlatformCmd.UNLOAD
...
def main():
    ...
    node.platform_request(PlatformCmd.PICKUP)
    ...
    for task in tasks:
        ...
        while True:
            ...
            node.arm_state_request(ArmStateCmd.DETECT_POSE)
            spin_for_time(node, 1)
            operation_char = node.update_text_info(task, False)
            ...
            if operation_char != "":
                ...
                node.update_text_info(PlatformCmd.UNLOAD, True)
