...
class PoseTransformer(Node):
    ...
    def listen_and_transform(self, src_pose) -> PoseStamped:
        try:
            ...
            link6_base_tf = self.tf_buffer.lookup_transform(target_frame = "base_link", source_frame = "camera", time = rclpy.time.Time())
...
class RedisGrabNode(Node):
    def __init__(self, tf_node):
        super().__init__('redis_grab_node')
        # Redis connection
    ...
    def poll_redis(self):
        ...
        if img_id != self.last_img_id:
            ...
            if time.time() - self.last_print >= 1:
                self.get_logger().info(f"{self.frame_cnt} fps, {1000 / max(self.frame_cnt,1):.1f} ms")
                self.last_print = time.time()
                self.frame_cnt = 0
